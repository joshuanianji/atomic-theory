module Playground exposing
    ...
    , Number
    , Animation, Game, Msg, delay, initAnimation, initGame, ring, subscriptionsAnimation, subscriptionsGame, updateAnimation, updateGame, viewAnimation, viewGame
    )

{-| Fiddle around with the source code to make it embeddable in elm ui
...
{-| Delays the current time
Helpful when making animations that need to be set back a couple seconds
circle black 100
|> fade (wave 0 1 4 (delay 3 time ))
-}
delay : Number -> Time -> Time
delay offset (Time time) =
...
viewAnimation : Animation -> (Time -> List Shape) -> Html.Html Msg
viewAnimation (Animation _ screen time) viewFrame =
    render screen (viewFrame time)


updateAnimation : Msg -> Animation -> Animation
updateAnimation msg model =
    animationUpdate msg model


subscriptionsAnimation : Animation -> Sub Msg
subscriptionsAnimation (Animation visibility _ _) =
    case visibility of
        E.Hidden ->
            E.onVisibilityChange VisibilityChanged
...
viewGame : Game memory -> (Computer -> memory -> List Shape) -> Html.Html Msg
viewGame (Game _ memory computer) viewMemory =
    render computer.screen (viewMemory computer memory)


updateGame : Msg -> Game memory -> (Computer -> memory -> memory) -> ( Game memory, Cmd Msg )
updateGame msg model updateMemory =
    ( gameUpdate updateMemory msg model
    , Cmd.none
    )
...
{-| Make rings:
ellipse =
ring brown 200 100 5
You give the color, the width and height, then the thickness So our `ellipse` example
is 200 pixels wide and 100 pixels tall, with 5 pixels in width
-}
ring : Color -> Number -> Number -> Number -> Shape
ring color width height thickness =
    Shape 0 0 0 1 1 (Ring color width height thickness)
...
-}
words : Color -> String -> Bool -> Shape
words color string isBold =
    Shape 0 0 0 1 1 (Words color string isBold)
...
renderShape (Shape x y angle s alpha form) =
    case form of
        ...
        Ring color width height thickness ->
            renderRing color width height thickness x y angle s alpha
        ...
        Words color string isBold ->
            renderWords color string x y angle s alpha isBold
...
renderRing : Color -> Number -> Number -> Number -> Number -> Number -> Number -> Number -> Number -> Svg msg
renderRing color width height thickness x y angle s alpha =
    ellipse
        (rx (String.fromFloat (width / 2))
            :: ry (String.fromFloat (height / 2))
...
renderWords : Color -> String -> Number -> Number -> Number -> Number -> Number -> Bool -> Svg msg
renderWords color string x y angle s alpha isBold =
    text_
        (textAnchor "middle"
